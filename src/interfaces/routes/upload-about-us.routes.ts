...
// Ruta absoluta a /public (fuera de src)
...
router.post('/', upload.single('file'), async (req: any, res: any) => {
  ...
  try {
    ...
    const result = await connection.request().query(`
      SELECT imagen_url FROM about_us WHERE id = 1
    `);
    const oldImageUrl: string | undefined = result.recordset[0]?.imagen_url;

    // 2. Eliminar imagen anterior si está en /public
    ...
    // 3. Actualizar imagen nueva en BD
    await connection.request().query(`
      UPDATE about_us SET imagen_url = '${newImageUrl}' WHERE id = 1
    `);

    res.json({ filename: file.filename });
  } catch (err) {
    console.error('Error al procesar la imagen:', err);
    res.status(500).json({ error: 'Error al procesar la imagen de about_us' });
  }
});

export { router as aboutUsUploadRouter };
